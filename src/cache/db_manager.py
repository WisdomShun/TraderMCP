    ...
    def log_trading_operation(
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute(
                ...
                (timestamp, operation, symbol, order_type, action, quantity, price, 
                 stop_loss, take_profit, reason, risk_checks, result, order_id, 
    ...
    def get_trading_logs(
        ...
        with self._get_connection() as conn:
            ...
            for row in rows:
                log = dict(row)
                # Parse JSON fields
                if log.get("risk_checks"):
                    log["risk_checks"] = json.loads(log["risk_checks"])
